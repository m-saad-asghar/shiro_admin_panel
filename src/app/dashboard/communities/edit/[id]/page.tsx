...
export default function EditCommunityPage(): React.JSX.Element {
  ...
  const buildImageUrl = React.useCallback((fileName: string | null | undefined): string => {
    if (!fileName) return '';
    ...
    return `${base}/storage/areas/${fileName}`;
  }, [apiBase]);
  ...
  const fetchCommunity = React.useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(msgFromServer || `Failed to fetch community. HTTP ${res.status}`);
      }
      ...
      setValues({
        ...
        main_image: buildImageUrl(payload.data.main_image),
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(firstError || msgFromServer || `Update failed. HTTP ${res.status}`);
      }

      showToast(
        payload?.message ? String(payload.message) : 'Community updated successfully.',
        'success'
      );
